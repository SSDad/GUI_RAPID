...
%     CT.MMn = single(mat2gray(CT.MM, CT.Lim));
  ...
  set(hFig_main, 'Name', [[data_main.version, ' - '], data_main.RadoncIDfromTable, '  ',...
...
% Axial
iso.iSlice.z = CT.idx_iso;
selected.iSlice.z = iso.iSlice.z;
...
iso.iSlice.x = round((CT.iso(1)-CT.xx(1))/CT.dx)+1;
selected.iSlice.x = iso.iSlice.x;
...
iso.iSlice.y = round((CT.iso(2)-CT.yy(1))/CT.dy)+1;
selected.iSlice.y = iso.iSlice.y;
