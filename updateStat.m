...
if ~data_main.flag.statData(selected.idxSS)
    axis(hAxis.Stat, 'off')
    for iT = 1:6
        set(data_main.hPlotObj.Stat(iT), 'xdata', [], 'ydata', []);
        set(data_main.hPlotObj.StatSub(iT), 'xdata', [], 'ydata', []);
    end
    ...
    if exist(statFileName, 'file')
     ...
     data_main.flag.statData(selected.idxSS) = true;
